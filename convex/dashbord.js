...
export const getUserbalances = query({
  handler: async (ctx) => {
    ...
    for (const e of expenses) {
      ...
      if (isPayer) {
        for (const s of e.splits) {
          ...
          youAreOwed += s.amount;
          // FIX 1: Property was `.owned` instead of `.owed`
    ...
    // Settlement transactions involving current user
    ...
    for (const [uid, { owed, owing }] of Object.entries(balanceByUser)) {
      ...
      net > 0 ? youAreOwedByList.push(base) : youOwedList.push(base);
    }

    youOwedList.sort((a, b) => b.amount - a.amount);
    ...
    return {
      ...
      oweDetails: { youOwe: youOwedList, youAreOwed: youAreOwedByList },
    };
  },
});
...
export const getMonthlySpending = query({
  handler: async (ctx) => {
    ...
    const monthlyTotals = {};
    for (let i = 0; i < 12; i++) {
      // FIX 2: Day argument was 'i' instead of '1' (which corrupted month dates)
    ...
    // FIX 3: Object has 'month' and 'total' properties, not 'amount'
          ...
          .filter((q) =>
            q.and(
              ...
              q.or(
                ...
                // FIX 4: Corrected spelling to 'receivedByUserId'
